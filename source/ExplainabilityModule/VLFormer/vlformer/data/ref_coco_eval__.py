...
class YTVISEvaluator(DatasetEvaluator):
    ...
    def _eval_predictions(self, predictions):
        ...
        for prediction in predictions:
            pred = prediction["segmentations"]
            gt = self._ytvis_api.loadAnnsFromExps(prediction["id"])[0]
            iou.extend(db_eval_iou(pred, gt))
            boundary.extend(db_eval_boundary(pred, gt))
    ...
    def save_single_output(self, root, prediction, obj_id):
        ...
        # dir = os.path.join(root, video_id, exp_id + "_" + prediction["expression"] + "_" + str(prediction["category_id"]), + str(prediction["score"]))
        # dir = os.path.join(root, video_id, exp_id, str(obj_id))
    ...
    def save_output(self, root, predictions):
        # os.makedirs(root, exist_ok=True)
        from PIL import Image 
        # self._logger.info("Annotations are not available for evaluation.")
        for prediction in predictions:
